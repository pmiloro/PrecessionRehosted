...
def readSolData(filename):
    #Grab and reshape the first line of data from the file so we don't need to know
    #the actual number of fields we're taking in (WARNING: super hacky)
    dOut = np.array([float(i) for i in open(filename, 'r').readline().rstrip().split(",")])
    ...
    with open(filename, 'r') as file:
        ...
        while l != "":
            ...
            #Make sure we skip the first and last lines so we don't include
            ...
            #Change this after the first loop so we don't accidentally skip
            #everything
            hasSkipped = True
    #Return the full set of separated data (with statement closes file automatically)
    return dOut
...
'''
def writeSolData(filename, initConditions, indVals, derivatives):
...
'''
Default R^N to R^2 mapping for the plotSolData() method; truncates the input
    array to pass only the first two fields directly as the "x" and "y" of the
    parameterization; expected to be replaced by additional methods for any
...
################################################################################
Return outArray -> A 2D (N, 2) array that's mapped the V columns of the input
    array to just two in the output array, in this case by simply truncation
'''
def defaultConversion(arr2):
    #Grab the first row and first two columns of the input array to serve as a
    #"seed" for the output array
    outArray = arr2[0, 0:2]
    #For all the other values (from the second row on)
    for index in range(1, arr2.shape[0]):
        ...
        outArray.append(arr2[index, 0:2], axis=0)
    #Return the now (N, 2) mapped array as the output
    return outArray
...
################################################################################
fullArray -> 2D Array of all "timesteps" and all data fields; rows = values for
    each timestep, columns = values for each field therein
...
'''
def plotSolData(fullArray, shouldParameterize, shouldShow, shouldSave,
 ...
 conversion=defaultConversion):
    ...
    if shouldParameterize:
        ...
        #data into a Cartesian coordinate path
        x, y = conversion(fullArray[1:, :])
        #Generate default unit labels for x and y if none were given
        if yUnits[0] == "":
            yUnits = ["meters" for i in range(2)]
        #Title the plot
        plt.title(plotTitle)
        #Label the x and y axes "label_name (units)"
        plt.xlabel(labels[0] + " (" + yUnits[0] + ")")
        plt.ylabel(labels[1] + " (" + yUnits[1] + ")")
    ...
    #If we're not making parametric plots
        ...
        for axis in range(n):
            ...
            plt.plot(tSteps, fullArray[:, axis])
...
################################################################################
writeSolData("test.dat", 1.0, np.linspace(0,5,100), dy_dx)
solData = readSolData("test.dat")
#print(solData[:, 0])
#showSolData(solData[:, 0], solData[:, 1])
plotSolData(solData, False, True, True, filename = "test.png", labels=["y"])
################################################################################